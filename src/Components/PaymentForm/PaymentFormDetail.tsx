...
const PaymentFormDetail = () => {
  ...
  useEffect(() => {
    ...
    if (resultCode === '0' && orderId) {  
      ...
      const checkPaymentStatus = async () => {
        try {
          const response = await axios.get(`http://18.205.19.89:8080/api/payments/status?orderId=${orderId}`);
          setPaymentStatus(prev => ({
            ...prev,
    ...
    if (fullName && phone && email && ageVerified && termsAccepted) {
      try {
        ...
        const response = await axios.post(
          'http://18.205.19.89:8080/api/bookings/create',
          bookingData
        );
  ...
  const handleMomoPayment = async () => {
    try {
      ...
      const response = await axios.post(
        'http://18.205.19.89:8080/api/payments/momo',
        paymentData
      );
